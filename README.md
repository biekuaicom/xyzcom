...
4. 命名空间名称填入：`XYZ_DATABASE`（或者你喜欢的任何名字），点击**添加**。
...
4. **变量名称 (Variable name)** 严格填入：`XYZ_DB` （必须大写，一字不差，这是代码里呼叫数据库的接头暗号）。
5. **KV 命名空间** 的下拉菜单中，选择你在“第二阶段”创建的那个数据库（比如 `XYZ_DATABASE`）。
6. 点击 **保存**。
...
在右侧的列表中，找到你之前创建的那个数据库名称（比如 XYZ_DATABASE），点击它进入查看。
